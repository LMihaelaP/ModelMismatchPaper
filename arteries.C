...
Tube :: Tube (double Length,
              ...
              double points, int init, double K,
              double f1, double f2, double f3, double m1, double Res1_in, double Res2_in,
              double CT_in):
	L(Length),
	rtop(topradius),
	rbot(botradius),
...

{
  ...
  double rgLr  = 1.0/rho/g/Lr;//4.0/3.0/rho/g/Lr;
  double rgLr2 = 1.0/rho/g/Lr2;
...
void Tube :: printPxt (FILE *fd, double t, int offset)
{
  ...
  for (int i=0; i<=N; i++)
  {
      ...
      fprintf (fd, "%13.10f %13.10f %15.10f %15.10f %15.10f %17.10f\n",
               t*Lr3/q, (i+offset)*h*Lr, (P(i,Anew[i])+p0)*rho*g*Lr/conv, Qnew[i]*q, Anew[i]*Lr2, c(i, Anew[i])*Fr2);
  }
}
...
double Tube :: P (int i, double A)
{
    double Ami = m1*A0[i];
    double p1i = p1[i];
    double theta = M_PI*((A/Ami)-(1.0/m1));
    double pnew = p1i*tan(theta);
  return pnew;
}
...
double Tube :: dPdA (int i, double A)
{
    ...
    double p1i = p1[i];
    double theta = M_PI*((A/Ami)-(1.0/m1));
    double dthdA = M_PI/Ami;

    double pnew = p1i*dthdA/sq(cos(theta));
  return pnew;
}
...
double Tube :: dPdx1(int i, double A)
{
    ...
    double dthdr0 = -2.0*A*sqrt(cu(M_PI/A0[i]))/m1;
    
    double pnew = (dp1dr0i*tan(theta)+p1i*dthdr0/sq(cos(theta)))*dr0dx[i];
...
double Tube :: B (int i, double A)
{
    ...
    double pnew = A*p1i*tan(theta) + Ami*p1i*log(cos(theta))/M_PI/Fr2;
...
double Tube :: dBdx1 (int i, double A)
{
    ...
    double theta = M_PI*((A/Ami)-(1.0/m1));
    double dp1dr0i = dp1dr0[i];
    double dthdr0 = -2.0*A*sqrt(cu(M_PI/A0i))/m1;
    
    double pnew = (A*(dp1dr0i*tan(theta)+p1i*dthdr0/sq(cos(theta)))+
                   m1*(2.0*sqrt(M_PI*A0i)*p1i*log(cos(theta))+A0i*dp1dr0i*log(cos(theta))-
                   A0i*p1i*tan(theta)*dthdr0)/M_PI)*dr0dx[i]/Fr2;
...
double Tube :: dBdx1h (int i, double A)
{
    ...
    double pnew = (A*(dp1dr0i*tan(theta)+p1i*dthdr0/sq(cos(theta)))+
                   m1*(2.0*sqrt(M_PI*A0i)*p1i*log(cos(theta))+A0i*dp1dr0i*log(cos(theta))-
                       A0i*p1i*tan(theta)*dthdr0)/M_PI)*dr0dxh[ip1]/Fr2;
  return pnew;
}
...
double Tube :: dBdAh (int i, double A)
{
    ...
    double pnew = (p1i*tan(theta)*(1.0-Ami*dthdA/M_PI)+
                   p1i*A*dthdA/sq(cos(theta)))/Fr2;
  return pnew;
}
...
double Tube :: d2BdAdxh (int i, double A)
{
    ...
    double dthdA = M_PI/Ami;
    double dthdr0 = -2.0*A*sqrt(cu(M_PI/A0i))/m1;
    double dthdr0dA = -2.0*sqrt(cu(M_PI/A0i))/m1;
    
    double pnew = (dp1dr0i*tan(theta)+p1i*dthdr0/sq(cos(theta))+
            A*(dthdA*dp1dr0i+2.0*p1i*tan(theta)*dthdr0+p1i*dthdr0dA)/sq(cos(theta))-
            ((2.0*m1*sqrt(M_PI*A0i)*p1i+Ami)*tan(theta)*dthdA+
             Ami*p1i*(dthdA*dthdr0/sq(cos(theta))+dthdr0dA))/M_PI)*dr0dxh[ip1]/Fr2;
    return pnew;
}
...
double Tube :: c (int i, double A) // The wave speed through aorta.
{
    ...
    double cnst = A*p1i*dthdA/sq(cos(theta))/Fr2;
...
void Tube :: bound_right (double k, double theta, double t)
{
    ...
    uR = qR/aR;
    k1 = 1 - k*(Res1+Res2)/(Res1*Res2*CT);
    k2 = k/(Res1*Res2*CT) - 1/Res1;
    
    cst = aR + HpR*k + (k1*Qold[N] + k2*P(N,Aold[N]) - qR)/(uR-cR);
    ...
    {
        f  = xr - cst - P(N,xr)/(uR-cR)/Res1;
        df = 1 - dPdA(N,xr)/(uR-cR)/Res1;
    ...
    // are updated.
    Anew[N] = xr;
    Qnew[N] = k1*Qold[N] + k2*P(N,Aold[N]) + P(N,Anew[N])/Res1;
...
void Tube :: bound_bif (double theta, double gamma)
{
    ...
    while (j <= ntrial && ok==false) // Find the zero
    {
        ...
        PN    = P(N,xb[10]);
        fvec[14] =  - PN + LD->P(0,xb[13]) ;
        fvec[15] =  - PN + RD->P(0,xb[16]) ;
        
        PN    = P(N,xb[9]);
        fvec[16] = - PN + LD->P(0,xb[12]) ;
        fvec[17] = - PN + RD->P(0,xb[15]) ;
